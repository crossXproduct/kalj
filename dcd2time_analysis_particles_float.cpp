...
void printParams(string path, int nfiles, int tfirst, int tint_sm, int tint_lg, int dtsave, int ntints, char t0_choice, int t0_dif, float q, float a, double qmax, double dt, int N_used);
...
int main() {
    ...
    int nfiles = 1, t0_sm = 0, tint_sm = 1, tint_lg = 1, ntints = 2, nt0s_max = 1, N_used;
...
// ========================================================================== //
    ...
    cout << "Number of particles to analyze: " << endl;
    cin >> N_used;
...
//      Initialize arrays
    ...
    vector <double> normalization(size_funcs);
...
//      Misc
    ...
    printParams(path, nfiles, t0_snapshot_sm*dtsave*delta_t, dt_snapshot_sm*dtsave*delta_t, dt_snapshot_lg*dtsave*delta_t, dtsave*delta_t, ntints, t0_choice, t0_snapshot_dif*dtsave*delta_t, q, a, qmax, delta_t, N_used);
    ...
    //      averaging
      ...
      //if fixed t0 difference (overlapping) was not specified by user, use current snapshot difference as *initial snapshot* spacing also (non-overlapping)
        ...
        //      final snapshot
            ...
            */
            ...
            if(cm_times.at(tf_snapshot) == false) {
                cm_x.at(tf_snapshot) = cm(xf, N_used);
                cm_y.at(tf_snapshot) = cm(yf, N_used);
                cm_z.at(tf_snapshot) = cm(zf, N_used);
        ...
        //      initial snapshot
            ...
            */
            ...
            if(cm_times.at(t0_snapshot) == false) {
                cm_x.at(t0_snapshot) = cm(x0, N_used);
                cm_y.at(t0_snapshot) = cm(y0, N_used);
                cm_z.at(t0_snapshot) = cm(z0, N_used);
        ...
        //      calculate function values
            ...
            //N=2;
            for(int i = 0; i < N_used; i++) { //loop over particles
            ...
            normalization.at(cindex) += double(N_used); //increment main normalization counter
            /*
            for(int j = 0; j < nqs; j++) { //loop over qs
