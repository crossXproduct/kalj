...
if gpu.communicator.rank == 0: print("r_cut_AA=",lj.r_cut[('A', 'A')])
epsilon_AB = 1.5
sigma_AB = 0.8
...
sim.run(0)
#if gpu.communicator.rank == 0: print("RANDOMIZED")
...
## Setup compressor
if gpu.communicator.rank == 0: print(sim.timestep)
ramp = hoomd.variant.Ramp(A=0, B=1, t_start=sim.timestep, t_ramp=10000)
initial_rho = sim.state.N_particles / sim.state.box.volume
if gpu.communicator.rank == 0: print("rho_i=",initial_rho)
initial_box = sim.state.box
if gpu.communicator.rank == 0: print("V_i=",initial_box.volume**(1.0/3.0))
final_box = hoomd.Box.from_box(initial_box)  # make a copy of initial_box
final_rho = 1.2 #same as Flenner & Co
if gpu.communicator.rank == 0: print("rho_f=",final_rho)
final_box.volume = sim.state.N_particles / final_rho
if gpu.communicator.rank == 0: print("V_f=",final_box.volume**(1.0/3.0)) #Kob & Andersen's is 9.4 in units of sigma_AA
...
## Run compressor
sim.run(t_r)
if gpu.communicator.rank == 0: print("COMPRESSED")
if gpu.communicator.rank == 0: print("Compression successful?",sim.state.box == final_box) #check compression success
if gpu.communicator.rank == 0: print(initial_box)
if gpu.communicator.rank == 0: print(final_box)
if gpu.communicator.rank == 0: print(sim.state.box)
#sim.operations.updaters.remove(box_resize) #remove compressor, don't need it anymore
if gpu.communicator.rank == 0: print("DOF=",thermodynamic_properties.degrees_of_freedom)
if gpu.communicator.rank == 0: print("Particles=",thermodynamic_properties.num_particles)
if gpu.communicator.rank == 0: print("KE=",thermodynamic_properties.kinetic_energy)
if gpu.communicator.rank == 0: print("kT=",thermodynamic_properties.kinetic_temperature)
if gpu.communicator.rank == 0: print("V_therm=",thermodynamic_properties.volume**(1.0/3.0))
